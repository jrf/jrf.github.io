...
# [Nested_Interval_Property](/notes/areas/mathematics/real_analysis/definitions/nested_interval_property/)
...
Because the intervals are nested, we see that every $b_{n}$ serves as an [upper_bound](/notes/areas/mathematics/real_analysis/definitions/upper_bound/) for $A$. Thus, we are justified in setting
...
Now, consider a particular $I_{n} = \left. \left\lbrack a_{n},b_{n} \right\rbrack \right.$. Because $x$ is an [upper_bound](/notes/areas/mathematics/real_analysis/definitions/upper_bound/) for $A$, we have $a_{n} \leq x$. The fact that each $b_{n}$ is an [upper_bound](/notes/areas/mathematics/real_analysis/definitions/upper_bound/) for $A$ and that $x$ is the least upper bound implies $x \leq b_{n}$.
