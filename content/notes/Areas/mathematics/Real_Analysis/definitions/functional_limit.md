...
Let $c$ be a [limit_point](/notes/areas/mathematics/topology/definitions/limit_point/) of the [domain](/notes/areas/mathematics/set_theory/definitions/domain/) of $f:A \rightarrow \mathbb{R}$. We say that $\lim\limits_{x \rightarrow c}f\left. (x) \right. = L$ provided that, for every $\epsilon$-neighborhood (see [epsilon-neighborhood](/notes/areas/mathematics/real_analysis/definitions/epsilon-neighborhood/)) $V_{\delta}\left. (c) \right.$ around $c$ with the property that for all $x \in V_{\delta}\left. (c) \right.$ partialerent from $c$ (with $x \in$ A), it follows that $f\left. (x) \right. \in V_{\epsilon}\left. (L) \right.$.
