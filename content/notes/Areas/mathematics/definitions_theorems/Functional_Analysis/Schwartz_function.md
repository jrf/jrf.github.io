...
A function $f \in C^{\infty}\left. \left( \mathbb{R}^{n} \right) \right.$is called a *Schwartz function* if it goes to zero as $|x| \rightarrow \infty$ faster than any inverse power of $x$, as do all its derivatives. That is, a [function](/notes/areas/mathematics/definitions_theorems/set_theory/function/) is a Schwartz function if there exist real constants $C_{\alpha\beta}$ such that
